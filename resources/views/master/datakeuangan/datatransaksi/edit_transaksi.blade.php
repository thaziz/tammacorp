                ...
                <div id="alert-tab" class="tab-pane fade in active">
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-12 col-sm-12 col-xs-12">
                      <form id="data-form">
                        <input type="hidden" readonly name="id" value="{{ $_GET['id'] }}">
                              ...
                              <div class="col-md-4 col-sm-9 col-xs-12">
                                <div class="form-group">
                                  <select class="form-control input-sm" name="akun_[]">
                                    <option :value="data_akun.id_akun" v-for="data_akun in akun" :selected="(n <= single_data.detail.length && data_akun.id_akun == single_data.detail[idx].td_acc)">
                                      @{{ data_akun.id_akun+' - '+data_akun.nama_akun }}
                                    </option>
                                  </select> 
                                </div>                                 
                              </div>
                              ...
                              <div class="col-md-3 col-sm-8 col-xs-11">
                                <div class="form-group">
                                  <select class="form-control input-sm" name="akun_dka[]" >
                                    <option value="D" :selected="(n <= single_data.detail.length && 'D' == single_data.detail[idx].td_posisi)">Debet</option>
                                    <option value="K" :selected="(n <= single_data.detail.length && 'K' == single_data.detail[idx].td_posisi)">Kredit</option>
                                  </select> 
                                </div>                                 
                              </div>
                        ...
                        <div class="form-group" align="right">
                          <input type="button" name="tambah_data" value="Update Data" class="btn btn-danger" @click="simpan" :disabled="btn_save_disabled">
                        </div>
      ...
      var vm = new Vue({
        ...
        methods: {
          ...
          simpan:function(evt){
            ...
            $('.overlay.main').fadeIn(200);
            $('#load-status-text').text('Harap Tunggu. Sedang Mengupdate Data');

            // if($('#data-form').data('bootstrapValidator').validate().isValid()){
              axios.post(this.baseUrl+'/master/keuangan/master_transaksi/update', 
                $('#data-form').serialize()
              ).then((response) => {
                console.log(response.data);
                if(response.data.status == 'berhasil'){
                  ...
                  $.toast({
                      text: 'Data Transaksi Tidak Dapat Ditemukan.',
                      showHideTransition: 'slide',
                      hideAfter: false,
                      position: 'top-right',
                      icon: 'error'
          ...
          getTransaksi: function(){
            axios.get(baseUrl+'/master/keuangan/master_transaksi/get_data_transaksi?ids={{ $_GET['id'] }}')
                  .then((response) => {
                    console.log(response.data);
                    this.single_data = response.data;

                    $('#nama').val(response.data.nama_transaksi);
                    $('#cash_flow').val(response.data.cashflow);
                    $('#type').val(response.data.type_transaksi);

                    $('#akun_1').val(response.data.detail[0].td_acc);
                    $('#akun_2').val(response.data.detail[1].td_acc);

                    this.akun_next = response.data.detail.length;
                  })
          },
